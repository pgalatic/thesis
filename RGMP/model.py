...
class Refine(nn.Module):
    ...
    def forward(self, f, pm):
        s = self.convFS1(f)
        sr = self.convFS2(functional.relu(s))
        sr = self.convFS3(functional.relu(sr))
        s = s + sr
        
        m = s + functional.upsample(pm, scale_factor=self.scale_factor, mode='bilinear')

        mr = self.convMM1(functional.relu(m))
        mr = self.convMM2(functional.relu(mr))
        m = m + mr
        return m
...
class Decoder(nn.Module):
    ...
    def forward(self, r5, x5, r4, r3, r2):
        ...
        x = self.GC(x) 
        r = self.convG1(functional.relu(x))
        r = self.convG2(functional.relu(r))
        m5 = x + r            # out: 1/32, 64
        m4 = self.RF4(r4, m5) # out: 1/16, 64
        m3 = self.RF3(r3, m4) # out: 1/8, 64
        m2 = self.RF2(r2, m3) # out: 1/4, 64

        p2 = self.pred2(functional.relu(m2))
        p3 = self.pred3(functional.relu(m3))
        p4 = self.pred4(functional.relu(m4))
        p5 = self.pred5(functional.relu(m5))
        
        p = functional.upsample(p2, scale_factor=4, mode='bilinear')
        
        return p, p2, p3, p4, p5


class RGMP(nn.Module):
    '''This is a wrapper for both the Encoder and Decoder classes.'''
    def __init__(self):
