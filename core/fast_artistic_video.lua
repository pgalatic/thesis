...
function func_load_style_image(i, pattern)
  assert(utils.file_exists(string.format(pattern, i)))
  return image.load(string.format(pattern, i), 3)
end
...
if (opt.continue_with > 2 and not opt.new_cut == 0) then
  ...
  assert(last_frame_stylized ~= nil)
